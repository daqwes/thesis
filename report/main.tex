...
\section{Convergence comparison}\label{section:baseline-comparison}
...
\begin{figure}[H]
    % Comes from experiments/iterations_no_avg
    \centering
    \includegraphics[width=0.7\textwidth]{figures/experiments/baseline/iters_acc_comp_iters_no_avg-1.png}
    \caption{Convergence plot of the prob-estimator vs Projected Langevin using mixed qubit data generation}
    \label{fig:conv-plot-mixed-DG}
\end{figure}

Here is the same experiment, but with separate qubit data generation:

\begin{figure}[H]
    % Comes from experiments/iterations_no_avg_sep
    \centering
    \includegraphics[width=0.7\textwidth]{figures/experiments/baseline/iters_acc_comp_iters_no_avg_sep.png}
    \caption{Convergence plot of the prob-estimator vs Projected Langevin using separate qubit data generation}
    \label{fig:conv-plot-sep-DG}
\end{figure}
...
A final element that is interesting to compare is how convergence is impacted by the qubit count. In this case, we will be running both algorithms with $n=\{3,4,5\}$:
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        % TODO: rerun if time allows to check which eta is needed for langevin
        \centering
...
With each qubit increase, we can see that convergence becomes more and more challenging. For $n=5$, we decreased $\eta=0.001/m$ as otherwise Langevin was not converging. This also explains why this curve is smooth during the burn-in period, as the steps we take are very small (on the order of $1\text{e-}7$). An interesting question to ask in this case is if Langevin is suitable for larger $n$, as one might expect no convergence happening with a very small $\eta$ (of course if it is required to make it smaller).
...
We can see that even with a fixed $\lambda$ for the prob-estimator, the information gain stagnates at around $1\text{e-}5$. We are not exactly sure why this occurs, as the original R code seems to have the same behaviour (and the authors do not provide metrics beyond $m=2000$). The only conclusion we can draw from this result is that it is an inherent property of the method.

\section{Impact of number of measurements}\label{section:comp-nb-meas}
The number of measurements, or observables, is another interesting thing to explore. While in general we assume a tomographically complete setup, here we will try to reduce the number of observables to see how both algorithms behave in this setup, and how many observables are needed in order to keep accuracy sufficiently high. We will be increasing \nmeas  from $1$ to either $4^n$ for mixed qubit or $3^n$ for separate qubit data generation, with the observables randomly chosen at each step. Here is what we obtain:
...
\section{Impact of knowledge of rank}\label{section:comp-nb}
