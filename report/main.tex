...
\begin{equation}
    \rho = \sum_{j} p_j \ket{\psi} \bra{\psi}
\end{equation}
...
It is described by a collection of orthogonal projective measurements $\{P_m\}$, where $P_mP_{m'}=\delta_{mm'}P_m$ is the projection onto the eigenspace of an \textit{observable} O associated to eigenvalue $m$. An observable corresponds to a physical property which can be measured, for example the position, momentum or spin of a particle. The matrix $O$ is Hermitian, allowing for a simple matrix decomposition: 
\begin{equation}
    O = \sum_m m P_m
...
A small example of such a collection are the projectors operators of the \textit{computational basis} ($\ket 0$ and $\ket 1$). The resulting matrices are $M_0 = \ket 0 \bra 0$ and $M_1 = \ket 1 \bra 1$, which indeed verify the completeness equation. If we have that $\ket \psi = a \ket 0 + b \ket 1$, then $p(0) = \braket{\psi|M_0^\dagger M_0|\psi} = \braket{\psi|M_0|\psi} = |a|^2$ (by idempotency of $M_0$).\medbreak
