...
$\mathcal{A}$ & Set of all observables\\
...
Well-established methods exist to approximate $\rho$. We can split the existing methods into 5 main categories: inversion, Pauli basis expansion, Maximum Likelihood, Compressed sensing and finally Bayesian methods, which are mostly sampling algorithms.\medbreak
...
The set $\{P_m\}$ relates to an \textit{observable} $M$, which a physical property of a quantum system that can be measured. It can for example be the position, momentum or the spin of a particle. The matrix $M$ is a Hermitian operator, which allows for a simple spectral decomposition:
\begin{equation}
    M = \sum_m m P_m
\end{equation}
...
\begin{equation}
    \pi(\boldsymbol{\theta}|\mb{D}) = \frac{\pi(\mb{D}|\boldsymbol{\theta})\pi(\boldsymbol{\theta})}{\int_{\boldsymbol{\theta}} \pi(\mb{D}|\boldsymbol{\theta}) \pi(\boldsymbol{\theta})d\boldsymbol{\theta}}
\end{equation}
...
\end{equation}
This corresponds to saying that our chain admits a \ti{stationary} distribution, or equivalently that the \ti{marginal} distributions of $\mb X^{(r)}$ and $\mb X^{(r+1)}$ are the same. The distribution $\pi(\mb x)$, called the \ti{target} density, corresponds to the distribution we want to sample from in the context of MCMC. It is important to note that not all chains admit a stationary distribution, a few properties must be respected:
\begin{itemize}
    \item \tb{Irreducibility}: a Markov chain is \ti{irreducible} if it explores the entire sample space, and does not get stuck in local regions. Formally, in the discrete case with $\mb X^{(r)}\in \mathbb{N}$, this corresponds to the condition 
    \begin{equation}
    ...
    \item \tb{Aperiodicity}: a Markov chain is \ti{aperiodic} if it does not have any deterministic cycles. Formally, we say that a discrete state $j$ is aperiodic if the set $\{r\geq1: [P^r]_{jj} > 0\}$ has no common divisor other than 1 (the matrix $P$ is the ) A chain is aperiodic if all of its states are aperiodic.
    ...
    \item \tb{Reversibility}: a Markov chain $(\mb X^{(r)})_{r\geq1}$ is \ti{reversible} if
    \begin{equation}
        \Pr(\mb X^{(r)} | \mb X^{(r+1)}) = \Pr(\mb X^{(r+1)} | \mb X^{(r)})
    \end{equation}
    Equivalently, the probability of transitioning between 2 states is the same. 
    \item \tb{Detailed balance}: a Markov chain with transition kernel $k$ satisfies the detailed balance if there exists a function $f$ such that  
    \begin{equation}
        k(\mb x|\mb x^*)f(\mb x) = k(\mb x^*|\mb x) f(\mb x^*)
    \end{equation}
    A very related therem states that if the chain $(\mb X^{(r)})_{r\geq1}$ satisfies the detailed balance with a PDF $\pi$, then $\pi$ is the invariant distribution for this chain and it is reversible. 
    \item \tb{Ergodicity}: a Markov chain in \ti{ergodic} if every state is ergodic. A state $j$ is ergodic if is aperiodic and recurrent. Intuitively, this property means that we can reach every state with a probability greater than 0.
\end{itemize}
