...
\end{equation}
with $A_m$ the Hermitian matrix characterizing the $m^{th}$ experiment and $\hat p_m \in [-1, 1]$ a measured value, corresponding to the data $\mb D$. The authors use mixed qubit data generation, for which more details can be found in section \ref{section:mixed-qub-dg}. \medbreak
...
\begin{table}[H]
    ...
    \begin{center}
        \begin{tabular}{|c|c|c|}
            \hline
            Proposal name & Parameter value & Error\\
            \hline\hline
...
For the MHGS algorithm, we decided to use the same set of proposals as for MHS, however in their scalar version:
...
\begin{table}[H]
    ...
    \begin{center}
        \begin{tabular}{|c|c|}
            ...
            Normal distribution, independent & $y^* \gets \sigma {\eta} \quad\text{with } {\eta} \sim N(0, 1)$\\
