...
As specified above, the prior is split in 2 parts. This is due to way the algorithm is structured: inside the main loop of Metropolis-Hastings (which goes over iterations $1\rightarrow T$), we also iterate over each dimension $1\rightarrow d$, first for $\gamma$, and then for $V$. This can be seen as doing Gibbs-within-Metropolis-Hastings. \medbreak
...
\begin{algorithm}[H]
    ...
    \For{$k\gets 1:T$}{ 
        ...
        $\tilde Y^{(k)} \gets \tilde Y^{(k-1)} - \eta^{(k)} \nabla f  (\tilde Y^{(k-1)}, \theta, \lambda) + \cfrac{\sqrt{2\eta_k}}{\beta} \tilde w^{(k)}$\;
...
\begin{algorithm}[H]
    ...
    \For{$k\gets 1:T$}{
        ...
        Sample $u \sim U(0, 1)$\;
        $\alpha \gets \min\left\{\log A_1(\tilde Y^*, \tilde Y^{(k-1)}, \theta, \lambda), 1\right\}$\;
        \eIf{$\log(u) \leq \alpha$}{
            \tcp*[l]{Accept $\tilde Y^*$}
...
\begin{algorithm}[H]
    ...
    %\underline{Prob-estimator}\;
    \Input{$T \in \mathbb{N}, Y_0 \in \mathbb{C}^{d \times r}, \theta \in \mathbb{R}, \lambda \in \mathbb{R}$}
    \Output{$\tilde Y \in \mathbb{R}^{2d\times 2r}$}
    ...
    \For{$k\gets 1:T$}{
        ...
        \For{$i\gets 1:2d$}{
            \For{$j\gets 1:2r$}{
                ...
                Sample $u \sim U(0, 1)$\;
                $\alpha \gets \min\left\{\log A_2(y^*, \tilde Y^{(k)}_{ij}, \theta, \lambda), 1\right\}$\;
                \eIf{$\log(u) \leq \alpha$}{
                    \tcp*[l]{Accept $y^*$}
...
\end{table}
We then re-ran the same gridsearch to obtain the following results:
...
As we can see, both new methods perform worse than the prob-estimator and Projected Langevin, even with a sufficiently good proposal. The MHGS algorithm seems to perform better, which validates the idea behind the prob-estimator, and other techniques such as \cite{Mai22, LLJL20}, which all sample one-dimensional distributions accross dimensions instead of directly sampling from the joint. \medbreak

The major conclusion to draw however from this result is that the prior is not a solution on its own, as using another algorithm with it does not lead to better or similar accuracy. This of course assumes that the proposal we use is correct and does not perform suboptimally. As the experiment is performed with 2 variations of Metropolis-Hastings, we must infer that it is not only related to the algorithm itself. This implies that if a new method is to be implemented using this prior, it is important to first choose the algorithm well, and then thoroughly benchmark its performance and tune its hyperparameters in order to gain confidence in its result. The prior itself does not provide any guarantees on accuracy. The prior also does not help with the convergence speed, as we can see MHGS taking a very long time to converge (and possibly not even converging). This means that in the context of our comparison, Langevin sampling is needed. Further work is required to verify if this property comes specifically from Langevin or the gradient in general, in which case methods such as Hamiltonian Monte Carlo may also be valid.

TODO: mention that convergence also comes at the expense of computation time 
Another plot that 
