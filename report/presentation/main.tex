...
\begin{frame}{Projected Langevin: prior}
    ...
    \begin{itemize}
        \item Introduced by Adel, Chrétien, Massart, Thompson in 2024 \cite{ACMT2024}
        ...
        % \end{equation}
        \begin{equation}
            \pi(Y) = \prod_{j=1}^r (\theta^2 + \smash{\underbrace{s_j(Y)^2}_{j\text{th singular value of } Y}})^{-(2d+r+2)/2}
        \end{equation}
...
\begin{frame}{Prob-estimator: full algorithm}
    \begin{algorithm}[H]\label{code:prob}
        ...
        $Y^{(t)} \gets Y^{(t-1)}$\;
                ...
                $\gamma^{(t)} \gets Y^{(t)} /(\sum_{k=1}^d Y_k^{(t)})$ 
...
\begin{frame}{Projected Langevin: full algorithm}
    \begin{algorithm}[H]
