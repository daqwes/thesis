...
def run_experiment(savefig=True):
    ...
    if savefig:
        plt.savefig(f"iters_acc_comp_time_no_avg_sep_rank1.pdf", bbox_inches="tight")
    plt.show()
    plt.close()
    ...
    if savefig:    
        plt.savefig(f"iters_acc_comp_iters_no_avg_sep_rank1.pdf", bbox_inches="tight")
    plt.show()
    plt.close()
    dump_run_information("run_iterations_no_avg_sep_rank1", {"iter": list(range(n_iter)), "acc_pl": accs_pl, "acc_prob": accs_prob})
