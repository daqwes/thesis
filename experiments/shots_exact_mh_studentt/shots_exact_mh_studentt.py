...
"""
Evolution of the accuracy wrt the number of shots, exact data generation, with mhs.
"""
def run_experiment(savefig=True):
    ...
    plt.title("Accuracy wrt shots, exact data, semilogy, with mhs")
    ...
    plt.xlabel("Number of shots [#]")
    plt.ylabel("$L_2$ squared error")
    plt.title("Accuracy wrt shots, exact data, loglog, with mhs")
    if savefig:
