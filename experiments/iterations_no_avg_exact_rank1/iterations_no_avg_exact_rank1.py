...
no running average, exact data generation, rho of rank1
"""
def run_experiment(savefig=True):
    ...
    plt.title("Accuracy wrt time, with burnin, exact data, rho of rank1")
    if savefig:
    ...
    plt.ylabel("$L_2$ squared error")
    plt.title("Accuracy wrt iters, with burnin, exact data, rho of rank1")
    if savefig:    
