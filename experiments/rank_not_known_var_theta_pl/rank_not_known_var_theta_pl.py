...
"""
Plot the accuracy of langevin vs prob wrt the rank of rho, with its rank assumed not known.
...
def run_experiment(savefig=True):
            ...
            for k in range(n_samples):
                ...
                if not test_run:
                    _, rho_approx_pl, _ = run_PL(
                        n, n_meas, n_shots, rho_rank, As, y_hat, n_iter, n_burnin, seed=None, init_point=init_point, eta_shots_indep=eta_shots_indep, theta=theta_pl
                    )
    ...
    plt.title("Accuracy wrt rank, with burnin, rank of rho not known, var theta for PL")
    if savefig:    
        plt.savefig(f"rank_not_known_var_theta_pl.pdf", bbox_inches="tight")
    plt.show()
    plt.close()
    if savefig:
        filename = "rank_not_known_var_theta_pl"
