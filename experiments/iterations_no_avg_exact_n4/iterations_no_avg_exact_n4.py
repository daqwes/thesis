...
no running average, exact data generation, number of qubits = 4
...
def run_experiment(savefig=True):
    ...
    plt.title("Accuracy wrt time, with burnin, exact data, n=4")
    if savefig:
    ...
    plt.ylabel("$L_2$ squared error")
    plt.title("Accuracy wrt iters, with burnin, exact data, n=4")
    if savefig:    
