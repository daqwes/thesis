...
no running average, rank 1 for true matrix
"""
def run_experiment(savefig=True):
    ...
    plt.title("Comparison of accuracy wrt time, with burnin, rho of rank1")
    if savefig:
        plt.savefig(f"iters_acc_comp_time_no_avg_rank1.pdf", bbox_inches="tight")
    plt.show()
    plt.close()
    ...
    plt.ylabel("$L_2$ squared error")
    plt.title("Comparison of accuracy wrt iters, with burnin, rho of rank1")
    if savefig:    
        plt.savefig(f"iters_acc_comp_iters_no_avg_rank1.pdf", bbox_inches="tight")
    plt.show()
    plt.close()

    dump_run_information("run_iterations_no_avg_rank1", {"iter": list(range(n_iter)), "acc_pl": accs_pl, "acc_prob": accs_prob})  
