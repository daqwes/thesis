...
def run_experiment(savefig=True):
    ...
    rhos_pl, _, cum_times_pl = run_PL(
        n, n_meas, n_shots, rho_type, As, y_hat, n_iter, n_burnin, seed=seed, running_avg=True
    )
    ...
    plt.title("Comparison of accuracy wrt time, with burnin, with avg for PL")
    if savefig:
        plt.savefig("iters_acc_comp_time_avg.pdf", bbox_inches="tight")
    plt.show()
    plt.close()
    ...
    plt.ylabel("$L_2$ squared error")
    plt.title("Comparison of accuracy wrt iters, with burnin, with avg for PL")
    if savefig:    
        plt.savefig(f"iters_acc_comp_iters_avg.pdf", bbox_inches="tight")
    plt.show()
    plt.close()

    dump_run_information("run_iterations_avg", {"iter": list(range(n_iter)), "acc_pl": accs_pl, "acc_prob": accs_prob})
