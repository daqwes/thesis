...
"""
Compare and evaluate the different proposals we could use for MHGS, with various scaling coefs. With running average.
"""
def run_experiment(savefig=True):
    ...
    for prop in proposals:
        for coef in scaling_coefs_prop:
            ...
            avg_acc_rate = 0
            for i in range(reps):
                rhos_mh_stt, rho_mh_stt, cum_times_mh_stt, acc_rate  = run_MH_gibbs_studentt(n, n_shots, As, y_hat, n_iter, n_burnin, seed = None, run_avg=run_avg, proposal_dist=prop, scaling_coef_prop = coef, use_prop_in_ratio=use_prop_in_ratio, log_transform=log_transform, init_point=init_point)
    ...
    results_df.to_csv("prop_search_mhgs.csv", sep=',')
