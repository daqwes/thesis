...
def run_experiment(savefig=True):
    ...
    plt.title("Accuracy wrt time, with burnin")
    if savefig:
        plt.savefig(f"iters_acc_comp_time_no_avg.pdf", bbox_inches="tight")
    plt.show()
    plt.close()
    ...
    plt.ylabel("$L_2$ squared error")
    plt.title("Accuracy wrt iters, with burnin")
    if savefig:    
        plt.savefig(f"iters_acc_comp_iters_no_avg.pdf", bbox_inches="tight")
    plt.show()
    plt.close()

    dump_run_information("run_iterations_no_avg", {"iter": list(range(n_iter)), "acc_pl": accs_pl, "acc_prob": accs_prob})
