...
no running average, separate qubit DG, number of qubits = 5
...
def run_experiment(savefig=True):
    ...
    plt.title("Accuracy wrt time, with burnin, sep DG, n=5")
    if savefig:
    ...
    plt.ylabel("$L_2$ squared error")
    plt.title("Accuracy wrt iters, with burnin, sep DG, n=5")
    if savefig:    
