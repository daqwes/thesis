...
This means that r = rank(rho) for langevin. Here, n=4.
"""
def run_experiment(savefig=True):
    ...
    plt.title("Accuracy wrt rank, with burnin, rank of rho known, n=4")
    if savefig:    
        plt.savefig(f"rank_known_n4.pdf", bbox_inches="tight")
    plt.show()
    plt.close()
    if savefig:
        # dump_run_information("rank_known_n4", {"rank": list(rho_ranks),  "acc_pl": accs_pl_avg, "acc_prob": accs_prob_avg})
        dump_run_information_from_tensors(accs_prob, accs_pl, {"d": list(rho_ranks), "samples": list(range(n_samples))}, "rank_known_n4")
