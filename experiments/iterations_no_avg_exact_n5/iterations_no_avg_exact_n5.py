...
no running average, exact data generation, number of qubits = 5
...
def run_experiment(savefig=True):
    ...
    plt.title("Accuracy wrt time, with burnin, exact data, n=5")
    if savefig:
        plt.savefig(f"iters_acc_comp_time_no_avg_exact_n5.pdf", bbox_inches="tight")
    plt.show()
    plt.close()
    ...
    plt.ylabel("$L_2$ squared error")
    plt.title("Accuracy wrt iters, with burnin, exact data, n=5")
    if savefig:    
        plt.savefig(f"iters_acc_comp_iters_no_avg_exact_n5.pdf", bbox_inches="tight")
    plt.show()
    plt.close()

    dump_run_information("run_iterations_no_avg_exact_n5", {"iter": list(range(n_iter)), "acc_pl": accs_pl, "acc_prob": accs_prob})
